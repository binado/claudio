...
Provide a ConfirmCommand implementation (CLI) or set 'skip_command_confirmation' to true in settings.",
    ...
    fn resolve_settings_value(value: &serde_json::Value) -> Result<serde_json::Value> {
        match value {
            serde_json::Value::String(s) => {
                Ok(serde_json::Value::String(env::substitute_env_vars(s)?))
            }
    ...
    mod tests {
        ...
        fn test_tilde_expansion_uses_os_home_not_claudio_home_dir() {
            ...
            unsafe {
                std::env::set_var(CLAUDIO_HOME_DIR_ENV_VAR, temp_dir.path());
            }
        ...
        fn test_resolve_settings_value_string_with_var() {
            ...
            let result =
                resolve_settings_value(&json!("prefix_${TEST_SETTING_VAR}_suffix")).unwrap();
